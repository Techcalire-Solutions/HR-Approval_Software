...
router.post('/save', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const financeEmails = await   findFinanceMail();

        // Temporarily saving team mail id manually
        const salesPerson = await UserPosition.findOne({ where: { userId: userId } });
        const salesPersonEmail = salesPerson ? salesPerson.projectMailId : req.user.email;
        
        const teamMailMap = {
            1: 'nishida.mrz@gmail.com',
        ...
        const teamEmail = salesPerson && salesPerson.teamId && teamMailMap[salesPerson.teamId] 
            ...
            : 'default_team@example.com';
        let ccEmails = [];
        if (salesPersonEmail) ccEmails.push(salesPersonEmail);
        if (financeEmails) {
            if (Array.isArray(financeEmails)) ccEmails.push(...financeEmails);
